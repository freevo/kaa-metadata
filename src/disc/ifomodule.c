...
static PyObject * ifoinfo_get_audio_tracks(ifo_handle_t *vtsfile, int id) {
    ...
    return Py_BuildValue("(ssii)", audiolang, audioformat, audiochannels, audiofreq);
}
...
static PyObject *ifoinfo_read_title(dvd_reader_t *dvd, ifo_handle_t *ifofile, int id) {
    tt_srpt_t *tt_srpt;
    ...
    /* chapters, angles, playtime, fps, format, aspect, width, height, audio, subtitles */
